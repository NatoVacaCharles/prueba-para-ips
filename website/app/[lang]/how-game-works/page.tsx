/*
 * SPDX-FileCopyrightText: 2026 jvondermarck
 * SPDX-License-Identifier: MIT
 */

import type { Metadata } from "next";
import { getDictionary } from "@/getDictionary";
import {Locale} from "../../../i18n-config";

export const metadata: Metadata = {
  title: "How it works · Dinosaur Exploder",
  description: "Learn the basic gameplay loop, controls, and goals of Dinosaur Exploder.",
};

export default async function HowGameWorksPage({params,}: {params: Promise<{lang: string}>;}) {
  const {lang} = await params;
  const dict = await getDictionary(lang as Locale);
  const { title, goal, gameplay, controls, demo, help } = dict.howGameWorks;
  
  return (
    <div className="max-w-4xl mx-auto w-full px-4 md:px-8 py-10">
      <h1 className="font-retro text-3xl md:text-4xl text-green-800 dark:text-green-200 mb-6">
        {title}
      </h1>

      <div className="space-y-6">
        <section className="bg-white/80 dark:bg-neutral-800/80 rounded-xl border border-green-200 dark:border-green-500/40 p-5 shadow-sm">
          <h2 className="font-retro text-xl text-green-800 dark:text-green-200 mb-3">{goal.title}</h2>
          <p className="font-mono text-green-950 dark:text-green-100 leading-relaxed">
            {goal.descr}
          </p>
        </section>

        <section className="bg-white/80 dark:bg-neutral-800/80 rounded-xl border border-green-200 dark:border-green-500/40 p-5 shadow-sm">
          <h2 className="font-retro text-xl text-green-800 dark:text-green-200 mb-3">{gameplay.title}</h2>
          <ol className="list-decimal pl-5 font-mono text-green-950 dark:text-green-100 space-y-2">
            {gameplay.steps.map((step: string, index: number) => (
      <li key={index}>{step}</li>
    ))}
          </ol>
          <p className="font-mono text-green-950 dark:text-green-100 mt-3">
            {gameplay.difficulty}
          </p>
        </section>

        <section className="bg-white/80 dark:bg-neutral-800/80 rounded-xl border border-green-200 dark:border-green-500/40 p-5 shadow-sm">
          <h2 className="font-retro text-xl text-green-800 dark:text-green-200 mb-3">{controls.title}</h2>
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse font-mono">
              <thead>
                <tr className="text-green-800 dark:text-green-200">
                  <th className="py-2 pr-3">{controls.headers.key}</th>
                  <th className="py-2">{controls.headers.action}</th>
                </tr>
              </thead>
              <tbody>
                // eslint-disable-next-line @typescript-eslint/no-explicit-any
                {controls.list.map((item: any, index: number) => (
                  <tr key={index} className="border-t border-green-200 dark:border-green-500/40 text-black dark:text-neutral-200">
                    <td className="py-2 pr-3 font-bold text-green-700 dark:text-green-400">{item.key}</td>
                    <td className="py-2">{item.action}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </section>

        <section className="bg-black/80 dark:bg-neutral-800/90 rounded-xl border-2 border-green-700 dark:border-green-500 p-5 shadow-sm">
          <h2 className="font-retro text-xl text-green-300 dark:text-green-400 mb-3">
            {demo.title}
          </h2>

          <video
            className="w-full max-h-[420px] object-contain rounded-lg border border-green-700 dark:border-green-500"
            autoPlay={false}
            loop
            controls
            preload="metadata"
          >
            <source
              src="https://github.com/user-attachments/assets/4b5a6ed4-2e68-4e12-a9c8-8a6c33178c5e"
              type="video/mp4"
            />
            {demo.error}
          </video>

          <p className="font-mono text-green-100 dark:text-green-200 mt-3 text-sm">
            {demo.description}
          </p>
        </section>

        <section className="bg-black/80 dark:bg-neutral-800/90 rounded-xl border-2 border-green-700 dark:border-green-500 p-5 shadow-sm">
          <h2 className="font-retro text-xl text-green-300 dark:text-green-400 mb-3"> {help.title} </h2>
          <p className="font-mono text-green-100 dark:text-green-200 leading-relaxed">
            {help.description}
          </p>
        </section>
      </div>
    </div>
  );
}
